...
export default {
  ...
  setup() {
    ...
    const initThree = () => {
      ...
      const plane = new THREE.Mesh(geometry, cloudMaterial)
      scene.add(plane)
    }

    const animate = (time) => {
      animationFrameId = requestAnimationFrame(animate)
      cloudMaterial.uniforms.iTime.value = time * 0.001
    ...
    const handleResize = () => {
      ...
      cloudMaterial.uniforms.iResolution.value.set(width, height)
    }

    const handleMouseMove = (event) => {
      const rect = container.value.getBoundingClientRect()
      cloudMaterial.uniforms.iMouse.value.set(
